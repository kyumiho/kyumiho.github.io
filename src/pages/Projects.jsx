...
function useTilt() {
  ...
  const onMouseMove = (e) => {
    ...
    el.style.transform = `perspective(900px) rotateX(${-y * 5}deg) rotateY(${x * 5}deg) scale(1.015)`
  }
  const onMouseLeave = () => {
  ...
  return (
    <motion.div variants={variants}>
      ...
      >
          ...
          <div className="flex items-start gap-3">
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                <h3 className="text-xl font-semibold" style={{ color: 'var(--clr-text-1)' }}>{project.title}</h3>
                <span className="px-2 py-0.5 rounded-full text-xs"
                  style={{ background: 'var(--clr-badge-bg)', color: 'var(--clr-badge)', border: '1px solid var(--clr-badge-border)' }}>
                  {project.badge}
                </span>
              </div>
            </div>
        ...
        <p className="text-sm leading-relaxed mb-8" style={{ color: 'var(--clr-text-3)' }}>
          {project.description}
        </p>
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          {project.link && (
            <a href={project.link} target="_blank" rel="noreferrer"
              ...
              onMouseEnter={e => e.currentTarget.style.color = 'var(--clr-link-hover)'}
              onMouseLeave={e => e.currentTarget.style.color = 'var(--clr-link)'}
            >
    ...
    <main className="min-h-screen flex items-center py-20 md:py-28 px-6">
      <div style={{ maxWidth: '896px', margin: '0 auto' }}>
        <motion.div initial="hidden" animate="show" variants={stagger} style={{ width: '100%' }}>
          <motion.p variants={fadeUp} className="text-xs font-medium tracking-[0.2em] uppercase text-center"
            style={{ color: 'var(--clr-text-4)', marginBottom: '20px' }}>
            Portfolio
          </motion.p>
          <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl font-bold text-center"
            style={{ color: 'var(--clr-text-1)', letterSpacing: '-0.03em', marginBottom: '16px' }}>
            Projects.
          </motion.h1>
          <motion.p variants={fadeUp} className="text-center text-lg"
            style={{ color: 'var(--clr-text-3)', marginBottom: '40px' }}>
            Things I've built and shipped.
          </motion.p>
          <motion.div variants={fadeUp} style={{ height: '1px', background: 'var(--clr-border)', marginBottom: '60px' }} />

          <div className="flex flex-col gap-5">
            {projects.map(project => (
              <ProjectCard key={project.title} project={project} variants={fadeUp} />
            ))}
          </div>
        </motion.div>
      </div>
